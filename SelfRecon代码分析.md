...
### Method
#### Pipeline
文章方法的流程如下图所示,作者会同时维护显式和隐式两种人体几何表达，在每一帧图像使用前向变形场 (Forward Deformation Field) 来生成当前帧空间的显式人体网格。变形场由两个部分组成 :                   
...
对于一段含有 帧的人体自转视频，作者首先采用 VideoAvatar[2] 的方法来生成 SMPL 人体模型的初始形状 和每一帧的姿态参数，再预定义一个姿态参数来生成初始的标准姿态空间的人体网格，并用来初始化隐式和显式表达。    

对于显式人体网格：将视频每一帧图像使用前向变形场 (Forward Deformation Field) 来生成当前帧空间的显式人体网格，作者使用可微的 Mask 损失、变形约束损失和骨架光滑损失来控制人体形状  
...
显式迭代后，标准人体网格T被更新为Ti，为了使隐式SDF与新的显式网格一致，作者设计了一致性损失(consistency loss)，通过最小化一致性损失可以更新隐式SDF,使SDF的零等值面和当前帧空间的显式网格的误差最小。最后将SDF零等值面表示的隐式曲面渲染出来就可以了。      

**简而言之，由于隐式曲面不能作为变形场的输入，所以隐式曲面不能直接变形，因此作者将显式网格(点+面)输入到变形场中变形后，再反过来用显式网格去更新隐式曲面，然后渲染隐式曲面，最终就可以看见姿态多样的人体了**。   
...
<img src="https://user-images.githubusercontent.com/84011398/202884991-e29c97a6-dab9-434b-8e62-e4bace948b59.png" width="350">
...
- 作者使用可变形 SDF 渲染方法，使用显式网格来帮助找到射线与变形后的零等值面的交点。首先从标准姿态空间Sη中提取一个标准人体模板网格T,通过变形场D可以将标准网格T变换得到第i帧图像对应网格Ti，理论上Ti是当前帧空间Si的近似，因此从相机c沿着方向v发出一条射线，这条射线与Ti的交点x 就是 这条射线与当前空间Si的交点的近似。通过重心坐标插值(Barycentric Interpolation)找到当前帧空间Si的点x在标准空间Sη中对应的点p,此时的点p就是 点x经过D^-1变形的结果的近似。
...
论文 IDR 提出可以使用一个 MLP 来估计渲染方程，并能在一定程度上将光照和材质解耦，对于刚性物体，通过Differentiable Non-rigid Ray-casting后，对于第i帧的光线、相机位置c、视点方向v、变形场Di的参数ψi和f的参数η唯一地确定了点p，因此可以把p作为表示这些参数的函数。因此就可以将SDF的零等值面上的点的位置、法向、视角方向和全局几何特征向量作为MLP的输入，估计该点沿着输入视角方向的颜色。  
...
### FastMinv
### MCAcc
- 由Monoport和ImplictSegCUDA 来进行Marching Cubes加速。
- Marching Cubes(MC)算法思想：  
用许多小正方体去对空间进行切分，然后找到等值面经过的小正方体，求出该小正方体的边与0等值面（0等值面就是空间场值为0的等值面）的交点，将这些交点按照一定的拓扑连接关系连接起来，作为0等值面在该六面体网格中的近似表示。   


### MCGpu
- 由MarchingCubes_CppCuda提供GPU MC模块(MCGpu),输入(dim-x,dim-y,dim-z)形状的SDF张量，输出由Marching Cubes构建的网格。   
...
### train.py:   
- 设置三种分辨率resolutions(coarse粗、medium中、fine细),主要是传到set_hierarchical_config()方法的Seg3dLossless中去了,有啥用呢？？  
- 从配置文件config.conf读取各种参数     
- optNet实例包含了SDF模型、LBS模型、CompositeDeformer(MLPTranslator + skinner)、相机camera、raster、render、shader    
- 调用set_hierarchical_config方法，根据分辨率resolutions分级地将config.conf配置文件的中train参数、loss参数传给optNet用于模型训练,将对应的batch_size传给dataloader    
...
- masks.hdf5 数据形状是(648, 1080, 1080)。mask作用就是提取目标区域，SelfRecon中的mask就是把人和背景进行分离。   
&emsp;即有648张大小为(1080,1080)的掩膜。由camera.pkl可知，每张图像大小为(width=1080, height=1080),所以每张mask的大小也为(1080,1080)        
<img src="https://user-images.githubusercontent.com/84011398/203249041-726aace8-9eb7-4bb8-9cc4-d22786c89c61.png"  width="500">      
