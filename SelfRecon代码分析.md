...
- 作者使用可变形 SDF 渲染方法，使用显式网格来帮助找到射线与变形后的零等值面的交点。首先从标准姿态空间Sη中提取一个标准人体模板网格T,通过变形场D可以将标准网格T变换得到第i帧图像对应网格Ti，理论上Ti是当前空间Si的近似，因此从相机c沿着方向v发出一条射线，这条射线与Ti的交点x 就是 这条射线与当前空间Si的交点 的近似。通过$(x^2 + x^y )^{x^y}+ x_1^2= y_1 - y_2^{x_1-y_1^2}$
